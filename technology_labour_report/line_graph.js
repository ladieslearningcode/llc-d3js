(() => {
	...
	// Find minium and maximum values for hours in the bikeshare array
	const minDuration = d3.min(bikeshare, (d) => d.duration_in_hours)
	const maxDuration= d3.max(bikeshare, (d) => d.duration_in_hours)
	...
	const xScale = d3.scaleBand()
				.domain(d3.range(bikeshare.length))
				.range([0, width])

				console.log(xScale(3))
				console.log(d3.range(bikeshare.length))

	// 6. Y scale will use the duration_in_hours datapoint
	...
	const line = d3.line()
			...
			.y((d) => yScale(d.duration_in_hours) )
	...
	svg.append("g")
			...
			.call(d3.axisBottom(xScale).tickFormat((d) => bikeshare[d].month)); // Create an axis component with d3.axisBottom
	...
	svg.append("path")
			.datum(bikeshare) // 10. Binds data to the line
	...
	svg.selectAll(".dot")
		...
		.enter().append("circle") // Uses the enter().append() method
			...
			.attr("cy", (d) => yScale(d.duration_in_hours) )
			.attr("r", 8)
			.attr("transform", "translate(30, 0)")
})();
